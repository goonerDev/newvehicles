...
//echo '<pre>';print_R($params);die;
		...
		//echo '<pre>';print_R($get_parts);die;
		if($get_parts){
...
//echo '<pre>';print_R($data);die;
	...
	function _substitute_tags_in_meta( $content, $replacement ) {
       ...
       function parts($ajaxCall = false, $pattern = false, $start = 0, $format = 'raw', $fulltext = true, $default_url = true) {
		$part_type = $this->uri->segment(3);
        $customer_group = empty( $this->customer_group ) ? $this->fixed_customer_group : $this->customer_group;
		$user_no = empty( $this->user_no ) ? $this->fixed_user_no : $this->user_no;
                $params = array(
			'pattern' => $pattern,
...
//echo '<pre>';print_R($params);die;
		...
		if( $ajaxCall == '404' )
		$data[ 'error_msg' ] = E_404_ERROR;
               $pagination_url = BASE_URL.'/catalog/parts/'.$part_type;
               $this->load->library( 'pagination' );
		$data[ 'pagination' ] = misc::pagination(
			$this->pagination,
			$pagination_url,
