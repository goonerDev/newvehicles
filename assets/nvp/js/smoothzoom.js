...
(function($) {
    $.fn.extend({
        smoothZoom: function(options) {
            ...
            function fitWidth() {
				var target_image=$('#lightwrap img');
                // var naturalWidth = $('#off-screen').width(),
                var naturalWidth = target_image.width(),
                    // naturalHeight = $('#off-screen').height(),
                    naturalHeight = target_image.height(),
                    viewportWidth = $(window).width(),
                    viewportHeight = $(window).height(),
                    // newWidth = (viewportWidth * 0.95),
					multiplicator=$('#product-detail-image')[0]!=undefined?1.5:3,
					// Added by Vony
                    newWidth = naturalWidth*multiplicator,
					ratio=1;
					// /Added
					...
					if(ratio!=1)
						...
						newHeight = naturalHeight*multiplicator;
					// /Added
                $('#lightwrap img').show();
                // if (newHeight > naturalHeight) {
				...
				$('#lightwrap img').animate({
					height: newHeight,
					width: newWidth,
					top: (newHeight>viewportHeight?0:(viewportHeight-newHeight)/2),
